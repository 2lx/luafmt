...
fn test_function() {
    ...
    // let result = parse("fn_name(a1, a2).field");
    // assert!(result.is_ok());
    // assert_eq!( &format!("{}", result.unwrap()), "fn_name(a1, a2).field");
    //
    // let result = parse("fn_name{a1, a2}.field");
    // assert!(result.is_ok());
    // assert_eq!( &format!("{}", result.unwrap()), "fn_name{ a1, a2 }.field");
    ...
    assert_eq!( &format!("{}", result.unwrap()), "fn = function(a) return a end");

    let result = parse("fn = function() return end");
    assert!(result.is_ok(), "{:?}", result);
    assert_eq!( &format!("{}", result.unwrap()), "fn = function() return end");
...
fn test_stat() {
    ...
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "a = \"32\"; b = { 3, 4 }; c = 45");
...
#[test]
fn test_for() {
    let result = parse(r#"for a in pairs(tbl) do
                            x.fn(a)
                        end"#);
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "for a in pairs(tbl) do x.fn(a) end");

    let result = parse("for a = 5, 1, -1 do x.fn(a) end");
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "for a = 5, 1, -1 do x.fn(a) end");

    let result = parse("for a = 1, 5 do x.fn(a) fn(b + 3) end");
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "for a = 1, 5 do x.fn(a); fn(b + 3) end");

    let result = parse("while a < 4 do fn(a) fn(b) break end");
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "while a < 4 do fn(a); fn(b); break end");

    let result = parse("local a, b repeat fn(a) fn(b) until a > b print(a, b)");
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "local a, b; repeat fn(a); fn(b) until a > b; print(a, b)");

    let result = parse(r#"local a, b
...
fn test_var() {
    ...
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "a, b, c = 4, 4 & 1, func(42)");
}
...
fn test_literal() {
    ...
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), "a = \"\\\"12'3'\\\"344\\\"\"");

    let result = parse(r#"a = '"12\'3\'"344"\\\''"#);
    assert!(result.is_ok());
    assert_eq!(&format!("{}", result.unwrap()), r#"a = '"12\'3\'"344"\\\''"#);
}
